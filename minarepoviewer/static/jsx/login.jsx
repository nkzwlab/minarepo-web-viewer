...
var loginAccount = function(email, passwd) {
  $.ajax({
    type: 'GET',
    url: '/users/login',
    data: {
...
var constants = {
  ...
  TOGGLE_LOGIN_BUTTON: 'TOGGLE_LOGIN_BUTTON'
};
...
var MinaRepoStore = Fluxxor.createStore({
  initialize: function() {
    ...
    this.bindActions(constants.TOGGLE_LOGIN_BUTTON, this.onToggleLoginButton);
  },
  getState: function() {
    return {
      email: this.email,
  ...
  onToggleLoginButton: function(data) {
    return;
  }
});
...
var LoginForm = React.createClass({
  ...
  onButtonClick: function(email, passwd) {
    return function(event) {
      flux.actions.onToggleLoginButton({
        email: email,
        passwd: passwd
      });
    }
  },
  onToggleUserCreateLink: function() {
    return function(event) {
      window.location.href = '/newuser';
    }
  },
  onToggleFogotPasswdLink: function() {
    return function(event) {
  ...
  render: function() {
     ...
     return <div className="row">
       <div className="small-6 small-centered columns mrv-btn-row">
         ...
         <button type="button" className="button" onClick={this.onButtonClick(email, passwd)}>ログイン</button>
         <div>
           <a onClick={this.onToggleUserCreateLink()}>新規登録</a>
         </div>
         <div>
           <a onClick={this.onToggleFogotPasswdLink()}>パスワードを忘れた</a>
         </div>
       </div>
     </div>;
  }
